...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('lp_nodes_bot.log'),
...
ADMIN_IDS = {int(id_) for id_ in os.getenv('ADMIN_IDS', '1338956004192157758').split(',') if id_.strip()}
ADMIN_ROLE_ID = int(os.getenv('ADMIN_ROLE_ID', '000000000000'))
...
DB_FILE = 'lp_nodes.db'
BACKUP_FILE = 'lp_nodes_backup.pkl'
PORT_RANGE_START = 20000
PORT_RANGE_END = 30000
INACTIVITY_TIMEOUT = int(os.getenv('INACTIVITY_TIMEOUT', '2592000'))  # 30 days in seconds
MAX_LOG_LINES = 100  # For viewing logs
...
SUPPORTED_OS = {
    ...
    "centos:7": "CentOS 7",
    "fedora:38": "Fedora 38",
    "ubuntu:24.04": "Ubuntu 24.04"  # New addition
}
...
RUN apt-get update && \\
    apt-get install -y systemd systemd-sysv dbus sudo \\
                       ...
                       docker.io openssh-server tmate ufw && \\
...
RUN echo '{welcome_message}' > /etc/motd && \\
    ...
    echo 'lp-nodes-{vps_id}' > /etc/hostname

# Install additional useful packages
RUN apt-get update && \\
    apt-get install -y neofetch htop nano vim wget git tmux net-tools dnsutils iputils-ping fail2ban && \\
    apt-get clean && \\
...
class Database:
    ...
    def _create_tables(self):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS vps_instances (
                ...
                last_activity TEXT  -- New field for inactivity monitoring
            )
        ''')
    ...
    def update_activity(self, token):
        ...
        self.update_vps(token, {'last_activity': now})
...
class LPNodesBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            self.loop.create_task(self.inactivity_monitor())  # New feature: inactivity monitor
    ...
    async def inactivity_monitor(self):
        """Monitor for inactive VPS and suspend them"""
        await self.wait_until_ready()
        while not self.is_closed():
            try:
                ...
                for token, vps in list(self.db.get_all_vps().items()):
                    if vps['status'] != 'running':
                        continue
                    last_activity_str = vps.get('last_activity')
                    ...
                    if (current_time - last_activity).total_seconds() > INACTIVITY_TIMEOUT:
                        try:
                            ...
                            container.stop()
                            self.db.update_vps(token, {'status': 'suspended'})
                            logger.info(f"Suspended inactive VPS {vps['vps_id']}")
                            # Notify owner
                            try:
                                owner = await self.fetch_user(int(vps['created_by']))
                                await owner.send(f"⚠️ Your VPS {vps['vps_id']} has been suspended due to inactivity. Connect to reactivate.")
                            except:
                                pass
                        except Exception as e:
                            logger.error(f"Error suspending inactive VPS {vps['vps_id']}: {e}")
            except Exception as e:
                logger.error(f"Error in inactivity_monitor: {e}")
...
def generate_ssh_password():
    ...
    return ''.join(random.choices(string.ascii_letters + string.digits + string.punctuation, k=16))  # Improved: longer and with punctuation
...
async def build_custom_image(vps_id, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        # Validate base_image
        if base_image not in SUPPORTED_OS:
            raise ValueError(f"Unsupported OS image: {base_image}")
        ...
        # Build the image
        image_tag = f"lp-nodes/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, vps_id=None, use_custom_image=True):
    ...
    try:
        ...
        if not use_custom_image:
            ...
            packages = [
                ...
                "openssh-server", "sudo", "ufw", "git", "docker.io", "systemd", "systemd-sysv", "fail2ban"
            ]
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up LP NODES customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up LP NODES customization...")
        ...
        hostname_cmd = f"echo 'lp-nodes-{vps_id}' > /etc/hostname && hostname lp-nodes-{vps_id}"
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ LP NODES VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ LP NODES VPS setup completed successfully!")
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 LP NODES VPS Bot Commands", color=discord.Color.blue())
...
`/backup_vps <vps_id>` - Backup your VPS
`/restore_vps <vps_id>` - Restore your VPS from backup
`/view_logs <vps_id>` - View VPS logs
`/add_port <vps_id> <port>` - Add port forwarding
""", inline=False)
...
`/search_vps <user_id>` - Search VPS by user
""", inline=False)
...
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
                           os_image: Literal[tuple(SUPPORTED_OS.keys())] = DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        try:
            container = bot.docker_client.containers.run(
                ...
                volumes={
                    f'lp-nodes-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                },
        ...
        await status_msg.edit(content="🔧 Container created. Setting up LP NODES environment...")
        await asyncio.sleep(5)
        ...
        try:
            ...
            embed.add_field(name="ℹ️ Note", value="This is a LP NODES VPS instance. You can install and configure additional packages as needed.", inline=False)
        ...
        if 'container' in locals():
            try:
                ...
                logger.error(f"Error cleaning up container: {cleanup_e}")
...
Last Activity: {vps.get('last_activity', 'Unknown')}
""",
                inline=False
            )
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="LP NODES Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling LP NODES bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "lp-nodes-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
@bot.hybrid_command(name='backup_vps', description='Backup your VPS')
@app_commands.describe(
    vps_id="ID of the VPS to backup"
)
async def backup_vps(ctx, vps_id: str):
    ...
    try:
        ...
        status_msg = await ctx.send(f"🔄 Backing up VPS {vps_id}...")
        
        backup_id = generate_vps_id()[:8]
        backup_time = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        backup_file = f"backups/{vps_id}_{backup_time}.tar"
        ...
        await status_msg.edit(content=f"✅ Backup created successfully!")
        await ctx.send(file=discord.File(zip_file))
        os.remove(zip_file)
        
    except Exception as e:
        logger.error(f"Error in backup_vps: {e}")
        await ctx.send(f"❌ Error backing up VPS: {str(e)}", ephemeral=True)

@bot.hybrid_command(name='restore_vps', description='Restore your VPS from backup')
@app_commands.describe(
    vps_id="ID of the VPS to restore"
)
async def restore_vps(ctx, vps_id: str):
    """Restore your VPS from backup"""
    await ctx.send("Please upload the backup ZIP file.", ephemeral=True)
    # Note: Handling file upload would require on_message event or interaction follow-up

@bot.hybrid_command(name='view_logs', description='View VPS logs')
@app_commands.describe(
    vps_id="ID of the VPS"
)
async def view_logs(ctx, vps_id: str):
    """View VPS logs"""
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or (vps["created_by"] != str(ctx.author.id) and not has_admin_role(ctx)):
        ...
        try:
            container = bot.docker_client.containers.get(vps["container_id"])
            logs = container.logs(tail=MAX_LOG_LINES).decode('utf-8')
            embed = discord.Embed(title=f"Logs for VPS {vps_id}", color=discord.Color.blue())
            embed.description = f"```{logs}```"
            await ctx.send(embed=embed, ephemeral=True)
        except Exception as e:
            await ctx.send(f"❌ Error viewing logs: {str(e)}", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in view_logs: {e}")
        await ctx.send(f"❌ Error: {str(e)}", ephemeral=True)

@bot.hybrid_command(name='add_port', description='Add port forwarding to VPS')
@app_commands.describe(
    vps_id="ID of the VPS",
    port="Port to forward"
)
async def add_port(ctx, vps_id: str, port: int):
    """Add port forwarding to VPS"""
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or vps["created_by"] != str(ctx.author.id):
            await ctx.send("❌ VPS not found or you don't have access to it!", ephemeral=True)
        ...
        try:
            ...
            success, output = await run_docker_command(vps["container_id"], ["ufw", "allow", str(port)])
            if success:
                await ctx.send(f"✅ Port {port} forwarded successfully!", ephemeral=True)
            else:
                await ctx.send(f"❌ Failed to forward port: {output}", ephemeral=True)
        except Exception as e:
            await ctx.send(f"❌ Error adding port: {str(e)}", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in add_port: {e}")
        await ctx.send(f"❌ Error: {str(e)}", ephemeral=True)

@bot.hybrid_command(name='search_vps', description='Search VPS by user (Admin only)')
@app_commands.describe(
    user_id="User ID to search"
)
async def search_vps(ctx, user_id: str):
    """Search VPS by user (Admin only)"""
    if not has_admin_role(ctx):
        await ctx.send("❌ You must be an admin to use this command!", ephemeral=True)
        return

    try:
        user_vps = bot.db.get_user_vps(user_id)
        
        if not user_vps:
            await ctx.send("No VPS found for this user.", ephemeral=True)
            return

        embed = discord.Embed(title=f"VPS Instances for User {user_id}", color=discord.Color.blue())
...
""",
        ...
        logger.error(f"Error in search_vps: {e}")
...
class VPSManagementView(ui.View):
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.followup.send("✅ LP NODES VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"LP NODES VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ LP NODES VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.followup.send("✅ LP NODES VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
    ...
    async def reinstall_os(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.response.send_modal(view, ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    def __init__(self, vps_id, container_id, original_message):
        ...
        for image, label in SUPPORTED_OS.items():
            self.add_os_button(label, image)
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling LP NODES VPS... This may take a few minutes.", ephemeral=True)
            
            await status_msg.edit(content="🔨 Building custom Docker image...")
            try:
                image_tag = await build_custom_image(self.vps_id, image)
            except Exception as e:
                await status_msg.edit(content=f"❌ Failed to build Docker image: {str(e)}")
                return
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    detach=True,
                    privileged=True,
                    hostname=f"lp-nodes-{self.vps_id}",
                    mem_limit=memory_bytes,
                    cpu_period=100000,
                    ...
                    ports={'22/tcp': str(vps['external_ssh_port'])},
                    volumes={
                        f'lp-nodes-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
                    restart_policy={"Name": "always"}
                )
            except Exception as e:
            ...
            bot.db.update_activity(token)
            await status_msg.edit(content="✅ LP NODES VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling LP NODES VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ LP NODES VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed.add_field(name="Last Activity", value=vps.get('last_activity', 'Unknown'), inline=True)
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ LP NODES VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
