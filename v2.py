...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('lpnodes_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('LPNodesBot')
...
DB_FILE = 'lpnodes.db'
BACKUP_FILE = 'lpnodes_backup.pkl'
                        ...
                        for pattern in MINER_PATTERNS:
                            if pattern in output:
                                ...
                                try:
                                    ...
                                    await owner.send(f"â ï¸ Your VPS {vps['vps_id']} has been suspended due to detected mining activity. Contact admin to unsuspend.")
                                except:
                                    pass
                                break
                    except Exception as e:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if isinstance(status_msg, discord.Interaction):
                await status_msg.followup.send(f"ð Waiting for package manager to be ready... (Attempt {attempt + 1}/{max_attempts})", ephemeral=True)
            else:
                await status_msg.edit(content=f"ð Waiting for package manager to be ready... (Attempt {attempt + 1}/{max_attempts})")
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    """Enhanced container setup with LP NODES customization"""
    try:
        # Ensure container is running
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("ð Checking container status...", ephemeral=True)
        else:
            await status_msg.edit(content="ð Checking container status...")
            
        container = bot.docker_client.containers.get(container_id)
        if container.status != "running":
            if isinstance(status_msg, discord.Interaction):
                await status_msg.followup.send("ð Starting container...", ephemeral=True)
            else:
                await status_msg.edit(content="ð Starting container...")
            container.start()
            await asyncio.sleep(5)
        ...
        if not use_custom_image:
            if isinstance(status_msg, discord.Interaction):
                await status_msg.followup.send("ð¦ Installing required packages...", ephemeral=True)
            else:
                await status_msg.edit(content="ð¦ Installing required packages...")
        ...
        # Setup SSH
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("ð Configuring SSH access...", ephemeral=True)
        else:
            await status_msg.edit(content="ð Configuring SSH access...")
        ...
        # Set LP NODES customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("ð¨ Setting up LP NODES customization...", ephemeral=True)
        else:
            await status_msg.edit(content="ð¨ Setting up LP NODES customization...")
        ...
        hostname_cmd = f"echo 'lpnodes-{vps_id}' > /etc/hostname && hostname lpnodes-{vps_id}"
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("âï¸ Setting resource limits...", ephemeral=True)
        else:
            await status_msg.edit(content="âï¸ Setting resource limits...")
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("â LP NODES VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="â LP NODES VPS setup completed successfully!")
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send(f"â {error_msg}", ephemeral=True)
        else:
            await status_msg.edit(content=f"â {error_msg}")
        return False, None, None

intents = discord.Intents.default()
intents.message_content = True
intents.members = True
bot = LPNodesBot(command_prefix='/', intents=intents, help_command=None)
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="LP NODES VPS"))
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="ð¤ LP NODES VPS Bot Commands", color=discord.Color.blue())
...
""", inline=False)
        ...
        await ctx.send("â An error occurred while processing your request.")
...
async def add_admin(ctx, user: discord.User):
    ...
    if not has_admin_role(ctx):
        await ctx.send("â You must be an admin to use this command!", ephemeral=True)
        return
    
    bot.db.add_admin(user.id)
    await ctx.send(f"â {user.mention} has been added as an admin!", ephemeral=True)
...
async def remove_admin(ctx, user: discord.User):
    ...
    if ctx.author.id != 1210291131301101618:  # Only the owner can remove admins
        await ctx.send("â Only the owner can remove admins!", ephemeral=True)
        return
    
    bot.db.remove_admin(user.id)
    await ctx.send(f"â {user.mention} has been removed from admins!", ephemeral=True)
...
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    if bot.db.is_user_banned(owner.id):
        await ctx.send("â This user is banned from creating VPS!", ephemeral=True)
        return

    if not ctx.guild:
        await ctx.send("â This command can only be used in a server!", ephemeral=True)
        return

    if not bot.docker_client:
        await ctx.send("â Docker is not available. Please contact the administrator.", ephemeral=True)
        return

    try:
        # Validate inputs
        if memory < 1 or memory > 5120000:
            await ctx.send("â Memory must be between 1GB and 512GB", ephemeral=True)
            return
        if cpu < 1 or cpu > 320000:
            await ctx.send("â CPU cores must be between 1 and 32", ephemeral=True)
            return
        if disk < 10 or disk > 10000000:
            await ctx.send("â Disk space must be between 10GB and 1000GB", ephemeral=True)
            return
        ...
        if len(containers) >= bot.db.get_setting('max_containers', MAX_CONTAINERS):
            await ctx.send(f"â Maximum container limit reached ({bot.db.get_setting('max_containers')}). Please delete some VPS instances first.", ephemeral=True)
            return
        ...
        if bot.db.get_user_vps_count(owner.id) >= bot.db.get_setting('max_vps_per_user', MAX_VPS_PER_USER):
            await ctx.send(f"â {owner.mention} already has the maximum number of VPS instances ({bot.db.get_setting('max_vps_per_user')})", ephemeral=True)
            return

        status_msg = await ctx.send("ð Creating LP NODES VPS instance... This may take a few minutes.")
        ...
        if use_custom_image:
            await status_msg.edit(content="ð¨ Building custom Docker image...")
            try:
                image_tag = await build_custom_image(vps_id, username, root_password, user_password, os_image)
            except Exception as e:
                await status_msg.edit(content=f"â Failed to build Docker image: {str(e)}")
                return

            await status_msg.edit(content="âï¸ Initializing container...")
            try:
                ...
                await status_msg.edit(content=f"â Failed to start container: {str(e)}")
                return
        else:
            await status_msg.edit(content="âï¸ Initializing container...")
            try:
                ...
                await status_msg.edit(content=f"â OS image {os_image} not found. Using default {DEFAULT_OS_IMAGE}")
                container = bot.docker_client.containers.run(
                    DEFAULT_OS_IMAGE,
                    detach=True,
                    privileged=True,
        ...
        await status_msg.edit(content="ð§ Container created. Setting up LP NODES environment...")
        await asyncio.sleep(5)
        ...
        await status_msg.edit(content="ð Starting SSH session...")
        ...
        try:
            embed = discord.Embed(title="ð LP NODES VPS Creation Successful", color=discord.Color.green())
            embed.add_field(name="ð VPS ID", value=vps_id, inline=True)
            embed.add_field(name="ð¾ Memory", value=f"{memory}GB", inline=True)
            embed.add_field(name="â¡ CPU", value=f"{cpu} cores", inline=True)
            embed.add_field(name="ð¿ Disk", value=f"{disk}GB", inline=True)
            embed.add_field(name="ð¤ Username", value=username, inline=True)
            embed.add_field(name="ð User Password", value=f"||{ssh_password}||", inline=False)
            if use_custom_image:
                embed.add_field(name="ð Root Password", value=f"||{root_password}||", inline=False)
            embed.add_field(name="ð Tmate Session", value=f"```{ssh_session_line}```", inline=False)
            embed.add_field(name="ð Direct SSH", value=f"```ssh {username}@<server-ip>```", inline=False)
            embed.add_field(name="â¹ï¸ Note", value="This is a LP NODES VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"â LP NODES VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
        except discord.Forbidden:
            await status_msg.edit(content=f"â I couldn't send a DM to {owner.mention}. Please ask them to enable DMs from server members.")
            
    except Exception as e:
        error_msg = f"â An error occurred while creating the VPS: {str(e)}"
        logger.error(error_msg)
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your LP NODES VPS Instances", color=discord.Color.blue())
...
""",
        ...
        await ctx.send(f"â Error listing VPS instances: {str(e)}")
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All LP NODES VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        if not vps:
            await ctx.send("â VPS not found!", ephemeral=True)
            return
        ...
        await ctx.send(f"â LP NODES VPS {vps_id} has been deleted successfully!")
    except Exception as e:
        logger.error(f"Error in delete_vps: {e}")
        await ctx.send(f"â Error deleting VPS: {str(e)}")
...
async def connect_vps(ctx, token: str):
    ...
    if not vps:
        await ctx.send("â Invalid token!", ephemeral=True)
        return
        
    if str(ctx.author.id) != vps["created_by"] and not has_admin_role(ctx):
        await ctx.send("â You don't have permission to access this VPS!", ephemeral=True)
        return
    ...
    try:
        try:
            ...
            await ctx.send("â VPS instance not found or is no longer available.", ephemeral=True)
            return
        ...
        embed = discord.Embed(title="LP NODES VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your LP NODES VPS
...
""".format(username=vps["username"]), inline=False)
        ...
        await ctx.send("â Connection details sent to your DMs! Use the Tmate command to connect to your LP NODES VPS.", ephemeral=True)
        
    except discord.Forbidden:
        await ctx.send("â I couldn't send you a DM. Please enable DMs from server members.", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in connect_vps: {e}")
        await ctx.send(f"â An error occurred while connecting to the VPS: {str(e)}", ephemeral=True)
...
async def vps_stats(ctx, vps_id: str):
    ...
    try:
        ...
        if not vps or (vps["created_by"] != str(ctx.author.id) and not has_admin_role(ctx)):
            await ctx.send("â VPS not found or you don't have access to it!", ephemeral=True)
            return

        try:
            container = bot.docker_client.containers.get(vps["container_id"])
            if container.status != "running":
                await ctx.send("â VPS is not running!", ephemeral=True)
                return
...
""", inline=True)
            ...
            await ctx.send(f"â Error checking VPS stats: {str(e)}", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in vps_stats: {e}")
        await ctx.send(f"â Error: {str(e)}", ephemeral=True)
...
async def change_ssh_password(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            await ctx.author.send(embed=embed)
            await ctx.send("â SSH password updated successfully! Check your DMs for the new password.", ephemeral=True)
        except Exception as e:
            await ctx.send(f"â Error changing SSH password: {str(e)}", ephemeral=True)
    except Exception as e:
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="LP NODES System Statistics", color=discord.Color.blue())
        ...
        await ctx.send(f"â Error getting system stats: {str(e)}", ephemeral=True)
...
"""
        ...
        await ctx.send(f"â Error getting system info: {str(e)}", ephemeral=True)
...
async def set_container_limit(ctx, max_limit: int):
    ...
    if ctx.author.id != 1210291131301101618:  # Only the owner can set limit
        await ctx.send("â Only the owner can set container limit!", ephemeral=True)
        return
    
    if max_limit < 1 or max_limit > 1000:
        await ctx.send("â Container limit must be between 1 and 1000", ephemeral=True)
        return
    
    bot.db.set_setting('max_containers', max_limit)
    await ctx.send(f"â Maximum container limit set to {max_limit}", ephemeral=True)
...
async def cleanup_vps(ctx):
    ...
    try:
        ...
        if cleanup_count > 0:
            await ctx.send(f"â Cleaned up {cleanup_count} inactive VPS instances!")
        else:
            await ctx.send("â¹ï¸ No inactive VPS instances found to clean up.")
    except Exception as e:
        logger.error(f"Error in cleanup_vps: {e}")
        await ctx.send(f"â Error during cleanup: {str(e)}", ephemeral=True)
...
async def vps_console(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            await ctx.send(f"â Console access to VPS {vps_id}:\n"
            ...
            await ctx.send(f"â Error accessing VPS console: {str(e)}", ephemeral=True)
    except Exception as e:
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your LP NODES VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Emembed(title="LP NODES Global Usage Statistics", color=discord.Color.blue())
...
async def migrate_vps(ctx, vps_id: str):
    ...
    try:
        ...
        status_msg = await ctx.send(f"ð Preparing to migrate VPS {vps_id}...")
        ...
        await status_msg.edit(content=f"ð Creating snapshot {backup_id} for migration...")
        ...
        await status_msg.edit(content=f"â Snapshot {backup_id} created successfully. Please download this file and import it on the new host: {backup_file}")
        
    except Exception as e:
        logger.error(f"Error in migrate_vps: {e}")
        await ctx.send(f"â Error during migration: {str(e)}", ephemeral=True)
...
async def emergency_stop(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            await ctx.send("â ï¸ Attempting to force stop the VPS... This may take a moment.", ephemeral=True)
            
            # Try normal stop first
            try:
                container.stop(timeout=10)
                bot.db.update_vps(token, {'status': 'stopped'})
                await ctx.send("â VPS stopped successfully!", ephemeral=True)
                return
            except:
                pass
            ...
            try:
                ...
                bot.db.update_vps(token, {'status': 'stopped'})
                await ctx.send("â VPS killed forcefully!", ephemeral=True)
            except subprocess.CalledProcessError as e:
                raise Exception(f"Failed to kill container: {e}")
            
        except Exception as e:
            await ctx.send(f"â Error stopping VPS: {str(e)}", ephemeral=True)
    except Exception as e:
...
async def emergency_remove(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            await ctx.send("â VPS removed forcefully!", ephemeral=True)
        except Exception as e:
            await ctx.send(f"â Error removing VPS: {str(e)}", ephemeral=True)
    except Exception as e:
...
async def suspend_vps(ctx, vps_id: str):
    ...
    try:
        ...
        if vps['status'] == 'suspended':
            await ctx.send("â VPS is already suspended!", ephemeral=True)
            return
        ...
        await ctx.send(f"â VPS {vps_id} has been suspended!")

        # Notify owner
        try:
            owner = await bot.fetch_user(int(vps['created_by']))
            await owner.send(f"â ï¸ Your VPS {vps_id} has been suspended by an admin. Contact support for details.")
        except:
            pass

    except Exception as e:
        logger.error(f"Error in suspend_vps: {e}")
        await ctx.send(f"â Error suspending VPS: {str(e)}")
...
async def unsuspend_vps(ctx, vps_id: str):
    ...
    try:
        ...
        if vps['status'] != 'suspended':
            await ctx.send("â VPS is not suspended!", ephemeral=True)
            return
        ...
        try:
            ...
            await ctx.send(f"â Error starting container: {str(e)}")
            return

        bot.db.update_vps(token, {'status': 'running'})
        await ctx.send(f"â VPS {vps_id} has been unsuspended!")

        # Notify owner
        try:
            owner = await bot.fetch_user(int(vps['created_by']))
            await owner.send(f"â Your VPS {vps_id} has been unsuspended by an admin.")
        except:
            pass

    except Exception as e:
        logger.error(f"Error in unsuspend_vps: {e}")
        await ctx.send(f"â Error unsuspending VPS: {str(e)}")
...
async def edit_vps(ctx, vps_id: str, memory: Optional[int] = None, cpu: Optional[int] = None, disk: Optional[int] = None):
    ...
    if memory is None and cpu is None and disk is None:
        await ctx.send("â At least one specification to edit must be provided!", ephemeral=True)
        return
    ...
    try:
        ...
        try:
            ...
            await ctx.send(f"â Error updating container: {str(e)}")
            return

        bot.db.update_vps(token, updates)
        await ctx.send(f"â VPS {vps_id} specifications updated successfully!")

    except Exception as e:
        logger.error(f"Error in edit_vps: {e}")
        await ctx.send(f"â Error editing VPS: {str(e)}")
...
async def ban_user(ctx, user: discord.User):
    ...
    await ctx.send(f"â {user.mention} has been banned from creating VPS!")
...
async def unban_user(ctx, user: discord.User):
    ...
    await ctx.send(f"â {user.mention} has been unbanned!")
...
async def backup_data(ctx):
    ...
    try:
        if bot.db.backup_data():
            await ctx.send("â Data backup completed successfully!", ephemeral=True)
        else:
            await ctx.send("â Failed to backup data!", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in backup_data: {e}")
        await ctx.send(f"â Error backing up data: {str(e)}", ephemeral=True)
...
async def restore_data(ctx):
    ...
    try:
        if bot.db.restore_data():
            await ctx.send("â Data restore completed successfully!", ephemeral=True)
        else:
            await ctx.send("â Failed to restore data!", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in restore_data: {e}")
        await ctx.send(f"â Error restoring data: {str(e)}", ephemeral=True)
...
async def reinstall_bot(ctx):
    ...
    if ctx.author.id != 1210291131301101618:  # Only the owner can reinstall
        await ctx.send("â Only the owner can reinstall the bot!", ephemeral=True)
        return

    try:
        await ctx.send("ð Reinstalling LP NODES bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "lpnodes-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        ...
        await ctx.send("â Bot reinstalled successfully! Restarting...")
        ...
        await ctx.send(f"â Error reinstalling bot: {str(e)}", ephemeral=True)
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"LP NODES VPS Management - {self.vps_id}", color=discord.Color.red())
        embed.add_field(name="Status", value="ð´ Container Not Found", inline=True)
        ...
        await interaction.response.send_message("â This VPS instance is no longer available. Please create a new one.", ephemeral=True)
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if vps['status'] == 'suspended':
                await interaction.followup.send("â This VPS is suspended. Contact admin to unsuspend.", ephemeral=True)
                return
            ...
            embed = discord.Embed(title=f"LP NODES VPS Management - {self.vps_id}", color=discord.Color.green())
            embed.add_field(name="Status", value="ð¢ Running", inline=True)
            ...
            await interaction.followup.send("â LP NODES VPS started successfully!", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"â Error starting VPS: {str(e)}", ephemeral=True)
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Emembed(title=f"LP NODES VPS Management - {self.vps_id}", color=discord.Color.orange())
            embed.add_field(name="Status", value="ð´ Stopped", inline=True)
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("â LP NODES VPS stopped successfully!", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"â Error stopping VPS: {str(e)}", ephemeral=True)
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"LP NODES VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("â LP NODES VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"â Error restarting VPS: {str(e)}", ephemeral=True)
    ...
    async def reinstall_os(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.response.send_message(f"â Error: {str(e)}", ephemeral=True)
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            token, vps = bot.db.get_vps_by_id(self.vps_id)
            if not vps:
                await interaction.response.send_message("â VPS not found!", ephemeral=True)
                return
            ...
            status_msg = await interaction.followup.send("ð Reinstalling LP NODES VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    detach=True,
                    privileged=True,
                    hostname=f"lpnodes-{self.vps_id}",
                    mem_limit=memory_bytes,
                    cpu_period=100000,
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'lpnodes-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
                await status_msg.edit(content=f"â OS image {image} not found. Using default {DEFAULT_OS_IMAGE}")
                container = bot.docker_client.containers.run(
                    DEFAULT_OS_IMAGE,
                    detach=True,
                    privileged=True,
            ...
            try:
                ...
                await status_msg.edit(content=f"â Container setup failed: {str(e)}")
                return
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"LP NODES VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="â LP NODES VPS reinstalled successfully!")
            ...
            try:
                await interaction.followup.send(f"â Error reinstalling VPS: {str(e)}", ephemeral=True)
            except:
                try:
                    channel = interaction.channel
                    await channel.send(f"â Error reinstalling LP NODES VPS {self.vps_id}: {str(e)}")
                except:
        ...
        try:
            ...
            if new_owner_input.startswith('<@') and new_owner_input.endswith('>'):
                ...
                if not new_owner_input.isdigit():
                    await interaction.response.send_message("â Please provide a valid user ID or @mention", ephemeral=True)
                    return
                new_owner_id = new_owner_input

            token, vps = bot.db.get_vps_by_id(self.vps_id)
            if not vps or vps["created_by"] != str(interaction.user.id):
                await interaction.response.send_message("â VPS not found or you don't have permission to transfer it!", ephemeral=True)
                return
            ...
            try:
                ...
                if bot.db.is_user_banned(new_owner.id):
                    await interaction.response.send_message(f"â {new_owner.mention} is banned!", ephemeral=True)
                    return

                # Check if new owner already has max VPS
                if bot.db.get_user_vps_count(new_owner.id) >= bot.db.get_setting('max_vps_per_user'):
                    await interaction.response.send_message(f"â {new_owner.mention} already has the maximum number of VPS instances ({bot.db.get_setting('max_vps_per_user')})", ephemeral=True)
                    return
            except:
                await interaction.response.send_message("â Invalid user ID or mention!", ephemeral=True)
                return

            bot.db.update_vps(token, {"created_by": str(new_owner.id)})

            await interaction.response.send_message(f"â LP NODES VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="LP NODES VPS Transferred to You", color=discord.Color.green())
            ...
            await interaction.response.send_message(f"â Error transferring VPS: {str(e)}", ephemeral=True)
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed = discord.Embed(title=f"LP NODES VPS Management - {vps_id}", color=discord.Color.blue())
        ...
        await ctx.send(f"â Error managing VPS: {str(e)}", ephemeral=True)
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or vps["created_by"] != str(ctx.author.id):
            await ctx.send("â VPS not found or you don't have permission to transfer it!", ephemeral=True)
            return

        if bot.db.is_user_banned(new_owner.id):
            await ctx.send("â This user is banned!", ephemeral=True)
            return

        # Check if new owner already has max VPS
        if bot.db.get_user_vps_count(new_owner.id) >= bot.db.get_setting('max_vps_per_user'):
            await ctx.send(f"â {new_owner.mention} already has the maximum number of VPS instances ({bot.db.get_setting('max_vps_per_user')})", ephemeral=True)
            return

        bot.db.update_vps(token, {"created_by": str(new_owner.id)})

        await ctx.send(f"â LP NODES VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
        ...
        await ctx.send(f"â Error transferring VPS: {str(e)}", ephemeral=True)

@bot.event
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send("â You don't have permission to use this command!", ephemeral=True)
    elif isinstance(error, commands.CommandNotFound):
        await ctx.send("â Command not found! Use `/help` to see available commands.", ephemeral=True)
    elif isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(f"â Missing required argument: {error.param.name}", ephemeral=True)
    else:
        logger.error(f"Command error: {error}")
        await ctx.send(f"â An error occurred: {str(error)}", ephemeral=True)
